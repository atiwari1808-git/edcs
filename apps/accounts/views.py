"""Auth + dashboard. Self-registration creates INACTIVE accounts; an admin
...
def activate_user(request, user_id):
    ...
    if u:
        ...
        log_action(request.user, "user.activated", "users", f"Activated '{u.username}'")
...
def dashboard(request):
    ...
    from apps.scheduler.services import filtered_bookings, booking_stats
    ...
    return render(request, "dashboard.html", {
        "nav": "dashboard", "bookings": rows, "stats": stats,
        "tools": ToolConfig.objects.filter(is_active=True),
        "query": q, "tool_filter": tool_filter, "status_filter": status_filter,
...
def dashboard_export(request):
    ...
    rows = filtered_bookings(
        request.GET.get("q", "").strip(), request.GET.get("tool", ""),
        request.GET.get("status", ""), request.GET.get("date_from", ""),
