...
def verify_page(request):
    ...
    the user's verification history. Result shown when ?run=<id>.
    Viewing is gated on verification.view; the Run button itself is
    additionally gated client-side (and server-side, on POST) by
    verification.execute — see templates/verify.html."""
    result_run = None
    if request.GET.get("run"):
        result_run = (ValidationRun.objects.filter(pk=request.GET["run"])
                      .prefetch_related("results").first())
    history = (ValidationRun.objects.filter(requested_by=request.user)
               .select_related("tool").prefetch_related("results")
               .order_by("-started_at")[:20])
    return render(request, "verify.html", {
        "nav": "verify",
        "tools": ToolConfig.objects.filter(is_active=True),
        "result_run": result_run, "history": history,
    })
...
def start(request):
    ...
    try:
        run = start_run(request.user, data.get("tool_key", ""),
                        data.get("jira_id", ""), data.get("repo_url", ""),
                        data.get("customer_name", ""), data.get("automation_name", ""))
    except ValidationError as e:
        return JsonResponse({"error": str(e)}, status=400)
    log_action(request.user, "validation.started", "verification",
              f"{run.tool.key} / {run.jira_id} (customer: {run.customer_name})")
...
def status_json(request, run_id):
    ...
    return JsonResponse({
        "status": run.status, "done": run.is_done, "summary": run.summary,
        "scanners": [{"scanner": r.scanner, "status": r.status,
                      "duration_ms": r.duration_ms} for r in run.results.all()],
...
def report_page(request, run_id):
    run = get_object_or_404(ValidationRun.objects.prefetch_related("results"),
                            pk=run_id)
